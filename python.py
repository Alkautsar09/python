        ...
        def faceList(name):
            with open('absensi.csv', 'r+') as f:
                ...
                if name not in nameList:
                    now = datetime.now()
                    dtString = now.strftime('%A-%B-%Y-%H-%M-%S')
                    f.writelines(f'\n{name},{dtString}')
        ...
        while True:
            ...
            for encodeFace,faceLoc in zip(encodeCurFrame,faceCurFrame):
                ...
                if matches[matchesIndex]:
                    name = classNames[matchesIndex].lower()
                    print(name)
                    cv2.rectangle(img,(x1,y1),(x2,y2),(0,255,0),2)
                    cv2.rectangle(img,(x1,y2-17),(x2,y2),(0,255,0),cv2.FILLED)
                    ...
                    cv2.rectangle(img,(x1,y2-17),(x2,y2),(0,255,0),cv2.FILLED)
                    cv2.putText(img,"Unknown",(x1+6,y2-6),cv2.FONT_HERSHEY_DUPLEX,0.5,(255,255,255),1)
